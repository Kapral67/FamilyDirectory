...
//      Get Caller List
            ...
            try {
                ...
                logger.log(format("Caller email: `%s`", callerEmail), INFO);
                ...
                logger.log(format("Caller email: `%s` authenticated as one of PK: `%s`", callerEmail, callerPKs), INFO);

            } catch (final NullPointerException | ClassCastException e) {
                final String message = "Caller Email Not Found";
...
//      Deserialization
            ...
            try {
                ...
                logger.log(format("Caller email: `%s` submitted invalid request", callerEmail), WARN);
...
//      Check Caller Authorization
            {
                ...
                for (final String callerPk : callerPKs) {
                    // TODO: SOLVE THE SPOUSE DILEMMA
                    if (callerPk.equals(input.getPrimaryKey())) {
                        isCallerAuthorized = true;
                        logger.log(format("Caller <PK,`%s`> <EMAIL,`%s`> Editing Self", callerPk, callerEmail), INFO);
                        break;
                    }
                }
...
//      Execute Transaction
            ...
            Optional.ofNullable(e.getCause())
                    .ifPresent(throwable -> logger.log(throwable.toString(), DEBUG));
